...
const PSDChart = ({ frequencies, power }: PSDChartProps) => {
  const data = useMemo(() => {
    const maxPoints = 400;
    ...
    return frequencies
      ...
      .map((f, idx) => ({
        ...
        power: power[idx * step],
      }));
  }, [frequencies, power]);

  return (
    <div className="oscilloscope-display p-2 h-full flex flex-col">
      <div className="section-title mb-1 px-2">Power Spectral Density</div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 5, right: 10, bottom: 20, left: 10 }}>
            ...
            <YAxis
              stroke="#5a6f7a"
              tick={{ fontSize: 9, fill: '#8fa3b0' }}
              label={{ value: 'Power', angle: -90, position: 'insideLeft', offset: 5, fontSize: 9, fill: '#8fa3b0' }}
            />
            <Tooltip
            ...
            />
            <Area type="monotone" dataKey="power" stroke="hsl(280 100% 65%)" fill="rgba(180,80,255,0.12)" strokeWidth={1.5} dot={false} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </div>
